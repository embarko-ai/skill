...
platform reference at `https://embarko.ai/docs.md` and `https://embarko.ai/troubleshoot.md`.
...
| Deploy API returns 422 "Unsupported storage pattern detected" | App calls `window.storage`, an API specific to Claude Artifacts' sandbox that doesn't exist on Embarko | Replace `window.storage` calls with SQLite (`better-sqlite3`) or PGlite (`@electric-sql/pglite`), writing under `DATA_DIR`, then redeploy — see `SKILL.md`'s Storage requirements section |
